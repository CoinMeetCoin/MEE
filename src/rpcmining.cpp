...
Value getposinfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("enabled", GetBoolArg("-posmee", false)));
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "MEE is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "MEE is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "MEE is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "MEE is downloading blocks...");
