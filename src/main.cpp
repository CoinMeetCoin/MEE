...
#if defined(NDEBUG)
# error "MEE cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 8 * 60;
...
const string strMessageMagic = "MEE Signed Message:\n";
...
bool AcceptToMemoryPool(CTxMemPool& pool, CTransaction &tx, bool fLimitFree, bool* pfMissingInputs)
{
    ...
    // MEE: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// MEE: find block wanted by given orphan block
...
// MEE: find last block index up to pindex
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // MEE: target change every block
    // MEE: retarget with exponential moving toward target spacing
    CBigNum bnNew;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal MEE miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // MEE: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // MEE: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // MEE: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // MEE: track money supply and mint amount info
...
// MEE: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // MEE: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // MEE: compute stake entropy bit for stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // MEE: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// MEE: attempt to generate suitable proof-of-stake
